...
## v4.5.44 (2026-09-17) - 删除首页起跳前多余的浮层提示

### 现象

首页点击 **CCTV直播 / 央视片库 / 央视栏目**，先出现「等待跳转...」约 **0.2~1 秒**，之后原生计时器才开启。两层提示叠在一起。

### 根因

`js/index.js` 的 `appChoose()` 在 `location.href` 之前先调 `_layer.wait(...)`，弹一个 JS 浮层；而原生在 `onPageStarted` 就已经接管（命中 `isVideoPage` 才显示计时遮罩）。两处做的是同一件事——提示"正在跳转"，JS 层抢先一步，于是先闪一下再被原生盖掉。

### 改法（`js/index.js`）

- 删掉 `appChoose()` 里两处浮层调用，只留 `window.location.href = dataUrl;`。
- 顺带删掉紧邻的死分支 `if (_utao_version <= 20)`：`index.html` 引的是 `base.js`（`_utao_version="21"`），条件恒不成立，且分支体与下方逻辑完全相同，删除后行为等价。

### 影响面

- **「支持我」入口保持现状**（zl 仍 `dsm.html`），本次未改。
- 央视网入口走 `_apiX.msgStr("activity","live")` 分支，本来就不弹浮层，无影响。
- `detailBase.js` 里那句「请耐心等待跳转」属详情页内的局部加载提示（有 `_layer.close(waitId)` 配对），不在首页链路，保留。
